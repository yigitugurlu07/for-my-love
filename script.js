...
async function startMusic(){
  ...
  music.volume=0;
  try{await music.play();musicStarted=true;sound.classList.remove("muted");let v=0;const f=setInterval(()=>{v=Math.min(.38,v+.02);music.volume=v;if(v>=.38)clearInterval(f)},90)}
  catch(e){sound.classList.add("muted")}
}
function openGift(){
...
sound.onclick=async()=>{
  if(!musicStarted){await startMusic();return}
  music.muted=!music.muted;sound.classList.toggle("muted",music.muted);$(".sound-icon").textContent=music.muted?"×":"♫"
};
